...
namespace Magedelight\SubscribenowPro\Model\ResourceModel\Report\PastRevenue;
...
class Collection extends \Magento\Sales\Model\ResourceModel\Report\Collection\AbstractCollection
{
    ...
    protected $tableForPeriod = [
        'daily'   => 'md_subscribenow_pastrevenue_aggregated_daily',
        'monthly' => 'md_subscribenow_pastrevenue_aggregated_monthly',
        'yearly'  => 'md_subscribenow_pastrevenue_aggregated_yearly',
    ];
        ...
        if (!$this->_selectedColumns) {
            if ($this->isTotals()) {
                ...
                $this->_selectedColumns = [
                    ...
                    $this->getAmountField() => 'SUM(' . $this->getAmountField() . ')',
                    'product_id' => 'MAX(product_id)',
                    'product_sku' => 'product_sku',
                    'product_name' => 'MAX(product_name)',
                    'subscription_ids' => 'GROUP_CONCAT(subscription_ids)',
                ];
    ...
    protected function _makeBoundarySelect($from, $to)
    {
        ...
        $cols[$this->getAmountField()] = 'SUM(' . $this->getAmountField() . ')';
        //$cols['subscription_ids'] = 'GROUP_CONCAT(subscription_ids)';
        ...
        $this->applyAdditionalFilters($select);
    ...
    protected function _applyAggregatedTable()
    {
        ...
        if (!$this->_period) {
            ...
            $select->where(new \Zend_Db_Expr($mainTable . '.product_sku IS NOT NULL'))->group(
                'product_sku'
            )->order(
        ...
        if (!$this->isTotals()) {
            $select->group(['period', 'product_sku']);
        }
