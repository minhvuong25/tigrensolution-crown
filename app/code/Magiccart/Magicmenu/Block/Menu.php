...
class Menu extends \Magento\Catalog\Block\Navigation
{
    ...
    public function drawMainMenu($isAccordion = false)
    {
        ...
        $dropdownIds = [];
        if($this->_sysCfg->general['dropdown'] != '')
        {
        ...
        if(isset($accordion['notIncludeNav'])){
            $hideCatIds  = explode(',', $accordion['notIncludeNav']);
        }

        foreach ($catListTop as $catTop){
            if($isAccordion && in_array($catTop->getEntityId(), $hideCatIds)) {
                continue;
            }
            $parentPositionClass = '';
