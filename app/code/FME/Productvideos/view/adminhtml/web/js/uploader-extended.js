...
define(
    ...
    return Element.extend(
        ...
        var p_type = file_type[file_type.length-1];
         ...
         valid= false;
            allowed.message = "Your Upload File Size :"+this.formatSize(file.size)+" is Greater than upload_max_filesize :"+file_type+" Or post_max_size :"+post_type+" in php.ini";
         } else {
            valid = true;
         }
            ...
            if (allowed.passed && valid) {
                $(e.target).fileupload('process', data).done(
                    function () {
                    data.submit();
                    }
                );
